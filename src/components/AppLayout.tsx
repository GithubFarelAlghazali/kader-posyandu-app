    ...
    <div className="min-h-screen bg-background flex flex-col">
      <div className="flex h-screen overflow-hidden">
        ...
        <aside className="hidden lg:flex flex-col w-72 border-r border-outline-variant/20 bg-white p-6 shadow-sm">
            ...
            {navItems.map((item) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === item.id 
                    ? 'bg-primary text-on-primary shadow-lg shadow-primary/20 font-black'
                    : 'text-on-surface hover:bg-surface-container hover:translate-x-1 font-medium'
                )}
              >
          ...
          <Button className="mt-auto w-full shadow-lg" onClick={() => setActiveTab('new-exam')}>
            <Plus className="w-6 h-6" strokeWidth={3} />
            <span className="font-black uppercase tracking-widest">New Examination</span>
          </Button>
        </aside>
        ...
        <div className="flex justify-around items-center h-20">
          {navItems.map((item) => (
            <button
              ...
              onClick={() => setActiveTab(item.id)}
              className={cn(
                'flex flex-col items-center justify-center transition-all min-w-[72px] h-full relative',
                activeTab === item.id ? 'text-primary' : 'text-on-surface'
              )}
            >
              {activeTab === item.id && (
                <motion.div 
                  layoutId="active-pill"
                  className="absolute top-0 w-12 h-1.5 bg-primary rounded-b-full shadow-[0_4px_8px_rgba(181,4,87,0.3)]"
                />
              )}
