...
const calendarData = [
  { day: 1, type: 'muted' }, { day: 2 }, { day: 3 }, { day: 4 }, { day: 5 }, { day: 6 }, { day: 7 },
  { day: 8 }, { day: 9, hasEvent: true }, { day: 10, isSelected: true }, { day: 11 }, { day: 12 }, { day: 13 }, { day: 14 },
  { day: 15, hasEvent: true }, { day: 16 }, { day: 17 }, { day: 18, isToday: true }, { day: 19 }, { day: 20 }, { day: 21 },
];
    ...
    <div className="space-y-10 max-w-3xl mx-auto">
      ...
      <div className="space-y-2">
        ...
        <p className="text-on-surface text-body-md font-bold opacity-60 uppercase tracking-widest leading-relaxed">
          Pantau antrean dan jadwal Posyandu Anda secara real-time.
        </p>
      </div>
      ...
      <motion.div 
        ...
        className="bg-white rounded-[40px] p-10 border border-outline-variant/30 shadow-clinical relative overflow-hidden group"
      >
        ...
        <Button size="lg" className="w-full relative z-10 py-6 text-lg shadow-xl hover:shadow-2xl font-black rounded-3xl">
          ...
          <div className="grid grid-cols-7 gap-y-6 gap-x-4">
            {calendarData.map((item, i) => (
              ...
              >
                ...
                {item.hasEvent && (
                  <span className={cn(
                    "absolute bottom-3 w-1.5 h-1.5 rounded-full transition-colors",
                    item.day === selectedDate ? "bg-on-primary" : "bg-primary"
                  )} />
                )}
              </button>
            ))}
      ...
      <div className="space-y-6">
        <h3 className="text-label-sm font-black text-on-surface uppercase tracking-[4px] opacity-60">
          EVENT: {selectedDate} OKTOBER 2023
        </h3>
          ...
          {selectedDate === 10 ? (
            ...
            >
              <div className="w-20 h-20 rounded-[24px] bg-primary-container text-on-primary-container flex items-center justify-center shrink-0 shadow-lg group-hover:scale-110 transition-transform">
                <span className="material-symbols-outlined text-[40px]" style={{ fontVariationSettings: "'FILL' 1" }}>vaccines</span>
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="text-headline-md font-black text-on-surface mb-2 group-hover:text-primary transition-colors">
                  Imunisasi Campak (MR)
                </h4>
            ...
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="py-16 border-4 border-dashed border-outline-variant/10 rounded-[40px] text-center bg-white/50 shadow-inner"
            >
        ...
        <div className="bg-primary text-on-primary rounded-[40px] p-8 shadow-2xl flex flex-col sm:row-span-1 sm:flex-row items-center justify-between gap-8 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-6 relative z-10">
             ...
             <div>
               ...
               <p className="text-label-sm font-medium opacity-80 leading-relaxed">
                 Dapatkan notifikasi cerdas 24 jam sebelum jadwal kunjungan Anda.
               </p>
             </div>
          </div>
          <button className="px-8 py-4 bg-white text-primary text-[11px] font-black uppercase tracking-[3px] rounded-2xl shadow-xl hover:scale-105 transition-all active:scale-95 relative z-10">
            Aktifkan Sekarang
          </button>
        </div>
      </div>
