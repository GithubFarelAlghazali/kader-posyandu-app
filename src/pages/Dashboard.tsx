    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
          title="High Risk"
          value="142"
          label="Requires immediate follow-up"
        ...
        <StatCard 
          ...
          value="8"
          label="Scheduled for this month"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <motion.div 
          ...
          className="lg:col-span-8 bg-white p-8 rounded-3xl border border-outline-variant/30 shadow-clinical flex flex-col min-h-[450px]"
        >
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={mockData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <XAxis 
                  ...
                  axisLine={false} 
                  tickLine={false} 
                  tick={{ fontSize: 12, fill: '#191c1d', fontWeight: 700 }}
                  dy={10}
                />
                <YAxis 
                  axisLine={false} 
                  tickLine={false} 
                  tick={{ fontSize: 12, fill: '#191c1d', fontWeight: 500 }}
                />
                <Tooltip 
        ...
        <div className="lg:col-span-4 space-y-8">
          <motion.div 
            ...
            className="bg-white p-8 rounded-3xl border border-outline-variant/30 shadow-clinical"
          >
            ...
            <div className="flex flex-col gap-5">
              {alerts.map((alert) => (
                <div 
                  key={alert.id}
                  className="flex items-center gap-4 p-4 rounded-2xl hover:bg-surface transition-all border border-transparent hover:border-outline-variant/30 group cursor-pointer"
                >
                  <div className={cn(
                    "w-12 h-12 rounded-full flex items-center justify-center shrink-0 shadow-sm",
                    alert.type === 'error' ? "bg-error text-on-error" : "bg-primary text-on-primary"
                  )}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      ...
                      <span className="text-[10px] text-on-surface font-black uppercase tracking-widest opacity-40 whitespace-nowrap ml-2">
                        {alert.time}
                      </span>
                    </div>
                    <p className={cn(
                      "text-label-sm font-bold truncate",
                      alert.type === 'error' ? "text-error" : "text-on-surface"
                    )}>
                      {alert.issue}
                    </p>
                  </div>
          ...
          <div className="bg-primary text-on-primary p-8 rounded-3xl shadow-2xl relative overflow-hidden group">
            ...
            <Button variant="secondary" className="w-full relative z-10 font-black shadow-lg">
