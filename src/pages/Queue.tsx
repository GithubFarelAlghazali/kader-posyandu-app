      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {categories.map((cat) => (
          <motion.div 
            key={cat.label}
            whileHover={{ y: -4 }}
            className="bg-white p-6 rounded-3xl border-l-8 border-primary shadow-clinical flex items-center gap-5 transition-all"
          >
      ...
      <div className="bg-white rounded-[32px] shadow-clinical border border-outline-variant/30 overflow-hidden">
        ...
        <div className="divide-y divide-outline-variant/10">
          {queueData.map((item) => (
            <div 
              key={item.id}
              className="p-8 flex flex-col md:flex-row items-center justify-between gap-8 hover:bg-surface-container-low/30 transition-all group"
            >
              <div className="flex items-center gap-8 w-full md:w-auto">
                ...
                <div className="min-w-0">
                  <p className="text-title-sm font-black text-on-surface truncate group-hover:text-primary transition-colors">
                    {item.name}
                  </p>
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className="text-label-sm text-on-surface font-bold opacity-60">
                      {item.reason}
                    </span>
                  </div>
                </div>
        ...
        <div className="p-8 bg-surface-container-low/30 text-center">
          <button className="text-label-sm font-black text-primary hover:underline decoration-2 underline-offset-4 transition-all">
            View Complete Analytical History
          </button>
        </div>
      </div>
    </div>
